Meteor.methods({
  removeSurvey(surveyId) {
    if (! surveyId) {
      throw new Meteor.Error("empty-survey-id", "Survey id cannot be empty");
    }

    let currentUser = Meteor.user();
    if (! currentUser) {
      throw new Meteor.Error("not-logged-in", "User must be logged in to remove a survey");
    }
    else {
      if (currentUser.profile && currentUser.profile.role === "administrator") {
        // remove survey from members' tasks list
        Meteor.users.update({ 'profile.role': "member" }, {
          $pull: { 'profile.tasks': { surveyId: surveyId } }
        }, {
          multi: true
        });

        // remove survey from members' notifications list
        Meteor.users.update({ 'profile.role': "member" }, {
          $pull: { 'profile.notifications': { surveyId: surveyId } }
        }, {
          multi: true
        });

        // remove survey if user is logged in and has a role of administrator
        return Surveys.remove(surveyId);
      }
      else {
        throw new Meteor.Error("not-authorized", "User not authorized to remove a survey");
      }
    }
  }
});
